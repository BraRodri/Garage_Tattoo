<x-app-layoutt>
    ...
    <div class="form-group">
        <a href="{{ route('regions.enter') }}" type="button" class="btn btn-blue"><i class="fa fa-plus"></i> Nuevo</a>
    </div>
    @endcan
    ...
    <div class="table-responsive">
        <table class="table table-bordered datatable order-table" id="table-3">
            ...
            <tbody>
                ...
                $class_status = $region->active == 1 ? 'success' : 'default';
                $text_status = $region->active == 1 ? 'Activo' : 'Inactivo';
                ?>
                <tr id="<?php echo $region->id; ?>">
                    <td width="5%">#<?php echo $region->code; ?></td>
                    <td><?php echo $region->description; ?></td>
                    <td width="7%" align="center"><a style="cursor: pointer;" class="change-status"
                            id="<?php echo $region->id; ?>"><span
                                class="badge badge-<?php echo $class_status; ?>"><?php echo $text_status; ?></span></a></td>
                    <td width="10%"><?php echo Application\Helper::dateFormatUser($region->updated_date);
                        ?></td>
                    <td width="10%"><?php echo $region->author; ?></td>
                    <td width="6%">

                        @can('regions.editar')
                        <a type="button" class="btn btn-sm btn-gold" data-toggle="tooltip" data-placement="top" title=""
                        data-original-title="Editar" href="{{ route('regions.edit', $region->id) }}"><i
                            class="fa fa fa-pencil-square-o"></i></a>
                        @endcan
                        
                        @can('regions.eliminar')
            ...
            jQuery(document).on("click", ".delete-register", function() {
                ...
                bootbox.confirm({
                    ...
                    callback: function(result) {
                        if (result == true) {
                            jQuery(location).attr('href','regions/delete/' + id);
                        }
                    }
                });
            });
