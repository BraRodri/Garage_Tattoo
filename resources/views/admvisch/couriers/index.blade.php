<x-app-layoutt>
    ...
    @can('couriers.agregar')
    <div class="form-group">
        <a href="{{route('couriers.enter')}}" type="button"
    ...
    <div class="table-responsive">
        <table class="table table-bordered datatable order-table" id="table-3">
            ...
            <tbody>
                ...
                $class_status = $courier->active == 1 ? 'success' : 'default';
                $text_status = $courier->active == 1 ? 'Activo' : 'Inactivo';
                ?>
                <tr id="<?php echo $courier->id; ?>">
                    <td width="5%">#<?php echo $courier->id; ?></td>
                    <td><?php echo $courier->title; ?></td>
                    <td width="7%" align="center"><a style="cursor: pointer;" class="change-status"
                            id="<?php echo $courier->id; ?>"><span
                                class="badge badge-<?php echo $class_status; ?>"><?php echo $text_status; ?></span></a></td>
                    <td width="10%"><?php echo
                        Application\Helper::dateFormatUser($courier->updated_date); ?></td>
                    <td width="10%"><?php echo $courier->author; ?></td>
                    <td width="6%">
                      
                        @can('couriers.editar')
                        <a type="button" class="btn btn-sm btn-gold" data-toggle="tooltip" data-placement="top" title=""
                        data-original-title="Editar"
                        href="{{route('couriers.edit',$courier->id)}}"><i
                            class="fa fa fa-pencil-square-o"></i></a>
                        @endcan
                        
                        @can('couriers.eliminar', Model::class)
            ...
            jQuery(document).on("click", ".delete-register", function() {
                ...
                bootbox.confirm({
                    ...
                    callback: function(result) {
                        if (result == true) {
                            jQuery(location).attr('href', 'couriers/delete/' + id);
                        }
                    }
                });
            });
