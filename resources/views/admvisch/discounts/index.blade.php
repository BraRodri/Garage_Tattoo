<x-app-layoutt>
    ...
    @can('discounts.agregar')
    <div class="form-group">
        <a href="{{ route('discounts.enter') }}" type="button" class="btn btn-blue"><i class="fa fa-plus"></i> Nuevo</a>
    </div>
    @endcan
    ...
    <div class="table-responsive">
        <table class="table table-bordered datatable" id="table-3">
            ...
            <tbody>
                ...
                $class_status = $discount->active == 1 ? 'success' : 'default';
                $text_status = $discount->active == 1 ? 'Activo' : 'Inactivo';
                ?>
                <tr id="<?php echo $discount->id; ?>">
                    <td width="5%">#<?php echo $discount->id; ?></td>
                    <td><?php echo $discount->title; ?></td>
                    <td><?php echo $discount->code; ?></td>
                    <td><?php echo \Application\Helper::dateFormatUser($discount->start_date, false) .
                        '
                        a ' .
                        \Application\Helper::dateFormatUser($discount->end_date, false); ?></td>
                    <td width="7%" align="center"><a style="cursor: pointer;" class="change-status"
                            id="<?php echo $discount->id; ?>"><span
                                class="badge badge-<?php echo $class_status; ?>"><?php echo $text_status; ?></span></a></td>
                    <td width="10%"><?php echo
                        Application\Helper::dateFormatUser($discount->updated_date); ?></td>
                    <td width="10%"><?php echo $discount->author; ?></td>
                    <td width="6%">

                        @can('discounts.editar')
                        <a type="button" class="btn btn-sm btn-gold" data-toggle="tooltip" data-placement="top" title=""
                        data-original-title="Editar" href="{{ route('discounts.edit', $discount->id) }}"><i
                            class="fa fa fa-pencil-square-o"></i></a>
                        @endcan
                        
                        @can('discounts.eliminar', Model::class)
    ...
    <x-slot name="js">
        ...
        <script type="text/javascript">
            ...
            jQuery(document).on("click", ".delete-register", function() {
                ...
                bootbox.confirm({
                    ...
                    callback: function(result) {
                        if (result == true) {
                            jQuery(location).attr('href', 'discounts/delete/' + id);
                        }
                    }
                });
            });
            ...
            jQuery(document).on("click", ".change-status", function() {
                ...
                var url = "{{ route('discounts.status') }}";
                var data = {
                    module_name: "{{ $module }}",
                    id: id
                }
