    ...
    switch ($this->connection->getAttribute(PDO::ATTR_DRIVER_NAME)) {
      ...
      case 'sqlsrv':
        $result =  $this->connection->query(
            "SELECT ISC.COLUMN_NAME, ISC.COLUMN_DEFAULT, ISC.DATA_TYPE, (SELECT 'PRI' FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE AS SKU WHERE OBJECTPROPERTY(OBJECT_ID(SKU.CONSTRAINT_SCHEMA + '.' + QUOTENAME(SKU.CONSTRAINT_NAME)), 'IsPrimaryKey') = 1 AND SKU.TABLE_NAME = '$table' AND SKU.TABLE_SCHEMA = 'dbo' AND SKU.COLUMN_NAME = ISC.COLUMN_NAME) AS COLUMN_KEY FROM INFORMATION_SCHEMA.COLUMNS AS ISC WHERE ISC.TABLE_SCHEMA = 'dbo' AND ISC.TABLE_NAME = '$table' ");
        $result->setFetchMode(PDO::FETCH_ASSOC);
        $meta = array();
        foreach ($result as $row) {
          $meta[$row['COLUMN_NAME']] = array(
