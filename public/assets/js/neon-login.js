...
;(function($, window, undefined)
{
	...
	$(document).ready(function()
	{
			...
			submitHandler: function(ev)
			{
				...
				*/
				...
				setTimeout(function()
				{
					...
					$.ajax({
						url: _base_url_ + _url_friendly_base_ + 'login/validate',
						method: 'POST',
						encoding:"UTF-8",
						dataType: 'json',
						data: {
						...
						success: function(response)
						{
							...
							setTimeout(function()
							{
								...
								if(login_status == 'invalid')
								{
									...
									$(".form-login-error p").html(response.login_message);
								...
								if(login_status == 'success')
								{
									...
									setTimeout(function()
									{
										var redirect_url = _base_url_;
										
										if(response.redirect_url && response.redirect_url.length)
										{
											redirect_url = _base_url_ + response.redirect_url;
										}
		...
		// Remember
		neonRemember.$container = $("#form_remember");


		// Login Form & Validation
		neonRemember.$container.validate({
			rules: {
				username: {
					required: true
				},
			...
			submitHandler: function(ev)
			{
				...
				setTimeout(function()
				{
					...
					// The form data are subbmitted, we can forward the progress to 70%
					neonRemember.setPercentage(40 + random_pct);

					// Send data to the server
					$.ajax({
						url: _base_url_ + _url_friendly_base_ + 'login/send',
						method: 'POST',
						encoding:"UTF-8",
						dataType: 'json',
						data: {
						...
						success: function(response)
						{
							...
							// Form is fully completed, we update the percentage
							neonRemember.setPercentage(100);
							...
							setTimeout(function()
							{
								...
								if(login_status == 'invalid')
								{
									$(".form-login-error").css('display', 'block');
									$(".form-login-error p").html(response.login_message);
									$(".form-login-error h3").css('display', 'block');
									neonRemember.resetProgressBar(true);
									$(".login-page").removeClass('logging-in');
								}
								else
								if(login_status == 'success')
								{
									...
									$(".form-login-error").css('display', 'block');
									$(".form-login-error p").html(response.login_message);
									$(".form-login-error h3").css('display', 'none');
									neonRemember.resetProgressBar(true);
									$(".login-page").removeClass('logging-in');
								}
		...
		// Remember Form Setup
		neonRemember.$body = $(".login-page");
		neonRemember.$login_progressbar_indicator = $(".login-progressbar-indicator h3");
		neonRemember.$login_progressbar = neonRemember.$body.find(".login-progressbar div");

		neonRemember.$login_progressbar_indicator.html('0%');

		if(neonRemember.$body.hasClass('login-form-fall'))
		{
			var focus_set = false;

			setTimeout(function(){
				neonRemember.$body.addClass('login-form-fall-init')

				setTimeout(function()
				{
					if( !focus_set)
					{
						neonRemember.$container.find('input:first').focus();
						focus_set = true;
					}
		...
		// Focus Class
		neonRemember.$container.find('.form-control').each(function(i, el)
		{
			var $this = $(el),
					$group = $this.closest('.input-group');
		...
		$.extend(neonRemember, {
			setPercentage: function(pct, callback)
			{
				...
				if(is_lockscreen)
				{
					neonRemember.$lockscreen_progress_indicator.html(pct);
					...
					TweenMax.to(o, .7, {
						...
						onUpdate: function()
						{
							neonRemember.$lockscreen_progress_indicator.html(o.pct + '%');
							drawProgress(parseInt(o.pct, 10)/100);
						},
						onComplete: callback
					});
				...
				// Normal Login
				neonRemember.$login_progressbar_indicator.html(pct);
				neonRemember.$login_progressbar.width(pct);

				var o = {
					pct: parseInt(neonRemember.$login_progressbar.width() / neonRemember.$login_progressbar.parent().width() * 100, 10)
				};
				...
				TweenMax.to(o, .7, {
					...
					onUpdate: function()
					{
						neonRemember.$login_progressbar_indicator.html(o.pct + '%');
					},
					onComplete: callback
				});
			},
			...
			resetProgressBar: function(display_errors)
			{
				TweenMax.set(neonRemember.$container, {css: {opacity: 0}});

				setTimeout(function()
				{
					TweenMax.to(neonRemember.$container, .6, {css: {opacity: 1}, onComplete: function()
					{
						neonRemember.$container.attr('style', '');
					}});

					neonRemember.$login_progressbar_indicator.html('0%');
					neonRemember.$login_progressbar.width(0);
					...
					if(display_errors)
					{
						...
						// Reset password fields
						neonRemember.$container.find('input[type="password"]').val('');
					}
